...
namespace crisp_controllers
{
                              ...
                              const rclcpp::Duration & /*period*/)
  {
    ...
    // TEST FILTER
    tau_d = exponential_moving_average(tau_previous, tau_d,
  ...
  void CartesianController::setStiffnessAndDamping()
  {
    ...
    damping.diagonal() << (params_.task.d_pos_x > 0 ? params_.task.d_pos_x : 2.0 * std::sqrt(params_.task.k_pos_x)),
    ...
    for (size_t i = 0; i < num_joints; i++)
    {
      ...
      { // simple revolute joint case
        q_pin[joint.idx_q()] = q[i];
                   ...
                   joint.shortname()))
      { // Then we are handling a continous
        // joint that is SO(2)
        q_pin[joint.idx_q()] = std::cos(q[i]);
        q_pin[joint.idx_q() + 1] = std::sin(q[i]);
      }
